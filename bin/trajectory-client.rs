...
use trajectory::client::{default_client_config, parse_socket_addr, require_resolvers};
...
async fn run() -> Result<()> {
    ...
    let mut config = default_client_config(listen, resolvers, domain);
    if let Some(ms) = keep_alive_ms {
        config.keep_alive_interval = if ms == 0 {
            std::time::Duration::from_millis(trajectory::protocol::KEEPALIVE_MS)
        } else {
            std::time::Duration::from_millis(ms)
        };
    }
    trajectory::client::run(config).await
}
