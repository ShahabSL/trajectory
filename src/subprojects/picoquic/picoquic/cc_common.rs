...
pub const PICOQUIC_MIN_MAX_RTT_SCOPE: ::core::ffi::c_int = 7 as ::core::ffi::c_int;
pub const PICOQUIC_SMOOTHED_LOSS_SCOPE: ::core::ffi::c_int = 32 as ::core::ffi::c_int;
pub const PICOQUIC_SMOOTHED_LOSS_FACTOR: ::core::ffi::c_double = 1.0f64 / 16.0f64;
pub const PICOQUIC_SMOOTHED_LOSS_THRESHOLD: ::core::ffi::c_double = 0.15f64;
#[no_mangle]
pub unsafe extern "C" fn picoquic_cc_get_sequence_number(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
) -> uint64_t {
    let mut sequence_number: uint64_t = 0;
    if (*cnx).is_multipath_enabled() != 0 {
        sequence_number = (*path_x).pkt_ctx.send_sequence;
    } else {
        sequence_number = (*cnx).pkt_ctx
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_cc_get_ack_number(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
) -> uint64_t {
    let mut highest_acknowledged: uint64_t = 0;
    if (*cnx).is_multipath_enabled() != 0 {
        highest_acknowledged = (*path_x).pkt_ctx.highest_acknowledged;
    } else {
        highest_acknowledged = (*cnx).pkt_ctx
            [picoquic_packet_context_application as ::core::ffi::c_int as usize]
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_cc_get_ack_sent_time(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
) -> uint64_t {
    let mut latest_time_acknowledged: uint64_t = 0;
    if (*cnx).is_multipath_enabled() != 0 {
        latest_time_acknowledged = (*path_x).pkt_ctx.latest_time_acknowledged;
    } else {
        latest_time_acknowledged = (*cnx).pkt_ctx
            [picoquic_packet_context_application as ::core::ffi::c_int as usize]
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_filter_rtt_min_max(
    mut rtt_track: *mut picoquic_min_max_rtt_t,
    mut rtt: uint64_t,
) {
    let mut x: ::core::ffi::c_int = (*rtt_track).sample_current;
    let mut x_max: ::core::ffi::c_int = 0;
    (*rtt_track).samples[x as usize] = rtt;
    (*rtt_track).sample_current = x + 1 as ::core::ffi::c_int;
    if (*rtt_track).sample_current >= PICOQUIC_MIN_MAX_RTT_SCOPE {
        (*rtt_track).is_init = 1 as ::core::ffi::c_int;
        (*rtt_track).sample_current = 0 as ::core::ffi::c_int;
    }
    x_max = if (*rtt_track).is_init != 0 {
        PICOQUIC_MIN_MAX_RTT_SCOPE
    } else {
        x + 1 as ::core::ffi::c_int
    };
    (*rtt_track).sample_min = (*rtt_track).samples[0 as ::core::ffi::c_int as usize];
    (*rtt_track).sample_max = (*rtt_track).samples[0 as ::core::ffi::c_int as usize];
    let mut i: ::core::ffi::c_int = 1 as ::core::ffi::c_int;
    while i < x_max {
        if (*rtt_track).samples[i as usize] < (*rtt_track).sample_min {
            (*rtt_track).sample_min = (*rtt_track).samples[i as usize];
        } else if (*rtt_track).samples[i as usize] > (*rtt_track).sample_max {
            (*rtt_track).sample_max = (*rtt_track).samples[i as usize];
        }
        i += 1;
    }
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_hystart_loss_test(
    mut rtt_track: *mut picoquic_min_max_rtt_t,
    mut event: picoquic_congestion_notification_t,
    mut lost_packet_number: uint64_t,
    mut error_rate_max: ::core::ffi::c_double,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    let mut next_number: uint64_t = (*rtt_track).last_lost_packet_number;
    if lost_packet_number > next_number {
        if next_number.wrapping_add(PICOQUIC_SMOOTHED_LOSS_SCOPE as uint64_t) < lost_packet_number {
            next_number = lost_packet_number.wrapping_sub(PICOQUIC_SMOOTHED_LOSS_SCOPE as uint64_t);
        }
        while next_number < lost_packet_number {
            (*rtt_track).smoothed_drop_rate *= 1.0f64 - PICOQUIC_SMOOTHED_LOSS_FACTOR;
            next_number = next_number.wrapping_add(1);
        }
        (*rtt_track).smoothed_drop_rate +=
            (1.0f64 - (*rtt_track).smoothed_drop_rate) * PICOQUIC_SMOOTHED_LOSS_FACTOR;
        (*rtt_track).last_lost_packet_number = lost_packet_number;
        match event as ::core::ffi::c_uint {
            1 => {
                ret = ((*rtt_track).smoothed_drop_rate > error_rate_max) as ::core::ffi::c_int;
            }
            2 => {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_hystart_loss_volume_test(
    mut rtt_track: *mut picoquic_min_max_rtt_t,
    mut event: picoquic_congestion_notification_t,
    mut nb_bytes_newly_acked: uint64_t,
    mut nb_bytes_newly_lost: uint64_t,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    (*rtt_track).smoothed_bytes_lost_16 = (*rtt_track).smoothed_bytes_lost_16.wrapping_sub(
        (*rtt_track)
            .smoothed_bytes_lost_16
            .wrapping_div(16 as uint64_t),
    );
    (*rtt_track).smoothed_bytes_lost_16 = (*rtt_track)
        .smoothed_bytes_lost_16
        .wrapping_add(nb_bytes_newly_lost);
    (*rtt_track).smoothed_bytes_sent_16 = (*rtt_track).smoothed_bytes_sent_16.wrapping_sub(
        (*rtt_track)
            .smoothed_bytes_sent_16
            .wrapping_div(16 as uint64_t),
    );
    (*rtt_track).smoothed_bytes_sent_16 = (*rtt_track)
        .smoothed_bytes_sent_16
        .wrapping_add(nb_bytes_newly_acked.wrapping_add(nb_bytes_newly_lost));
    if (*rtt_track).smoothed_bytes_sent_16 > 0 as uint64_t {
        (*rtt_track).smoothed_drop_rate = (*rtt_track).smoothed_bytes_lost_16
            as ::core::ffi::c_double
            / (*rtt_track).smoothed_bytes_sent_16 as ::core::ffi::c_double;
    } else {
        (*rtt_track).smoothed_drop_rate = 0 as ::core::ffi::c_int as ::core::ffi::c_double;
    }
    match event as ::core::ffi::c_uint {
        0 => {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_hystart_test(
    mut rtt_track: *mut picoquic_min_max_rtt_t,
    mut rtt_measurement: uint64_t,
    mut packet_time: uint64_t,
    mut current_time: uint64_t,
    mut is_one_way_delay_enabled: ::core::ffi::c_int,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    if current_time
        > (*rtt_track)
            ...
            .wrapping_add(1000 as uint64_t)
    {
        ...
        if (*rtt_track).is_init != 0 {
            let mut delta_max: uint64_t = 0;
            if (*rtt_track).rtt_filtered_min == 0 as uint64_t
                || (*rtt_track).rtt_filtered_min > (*rtt_track).sample_max
            {
                (*rtt_track).rtt_filtered_min = (*rtt_track).sample_max;
            }
            delta_max = (*rtt_track).rtt_filtered_min.wrapping_div(4 as uint64_t);
            if (*rtt_track).sample_min > (*rtt_track).rtt_filtered_min {
                if (*rtt_track).sample_min > (*rtt_track).rtt_filtered_min.wrapping_add(delta_max) {
                    (*rtt_track).nb_rtt_excess += 1;
                    if (*rtt_track).nb_rtt_excess >= PICOQUIC_MIN_MAX_RTT_SCOPE {
                        ret = 1 as ::core::ffi::c_int;
                    }
                }
            } else {
                (*rtt_track).nb_rtt_excess = 0 as ::core::ffi::c_int;
            }
        }
    }
    return ret;
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_hystart_increase(
    mut path_x: *mut picoquic_path_t,
    mut rtt_filter: *mut picoquic_min_max_rtt_t,
    mut nb_delivered: uint64_t,
) {
    (*path_x).cwin = (*path_x).cwin.wrapping_add(nb_delivered);
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_cc_increased_window(
    mut cnx: *mut picoquic_cnx_t,
    mut previous_window: uint64_t,
) -> uint64_t {
    let mut new_window: uint64_t = 0;
    if (**(*cnx).path.offset(0 as ::core::ffi::c_int as isize)).rtt_min as ::core::ffi::c_ulonglong
        <= PICOQUIC_TARGET_RENO_RTT
    {
        new_window = previous_window.wrapping_mul(2 as uint64_t);
    } else {
        let mut w: ::core::ffi::c_double = previous_window as ::core::ffi::c_double;
        w /= PICOQUIC_TARGET_RENO_RTT as ::core::ffi::c_double;
        w *= (if (**(*cnx).path.offset(0 as ::core::ffi::c_int as isize)).rtt_min
