...
#[no_mangle]
pub unsafe extern "C" fn picoquic_format_token(
    ...
    mut token_length: uint16_t,
) -> *mut picoquic_stored_token_t {
    let mut token_size: size_t = (::core::mem::size_of::<picoquic_stored_token_t>() as size_t)
        .wrapping_add(sni_length as size_t)
        .wrapping_add(1 as size_t)
        .wrapping_add(ip_addr_length as size_t)
        .wrapping_add(1 as size_t)
        .wrapping_add(token_length as size_t);
    let mut stored: *mut picoquic_stored_token_t =
        malloc(token_size) as *mut picoquic_stored_token_t;
    if !stored.is_null() {
        let mut next_p: *mut uint8_t = (stored as *mut uint8_t)
            .offset(::core::mem::size_of::<picoquic_stored_token_t>() as usize as isize);
        memset(
        ...
        (*stored).sni = next_p as *const ::core::ffi::c_char;
        ...
        *c2rust_fresh0 = 0 as uint8_t;
        ...
        next_p = next_p.offset(ip_addr_length as ::core::ffi::c_int as isize);
        let c2rust_fresh1 = next_p;
        next_p = next_p.offset(1);
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_serialize_token(
    mut token: *const picoquic_stored_token_t,
    ...
    required_length = ((8 as ::core::ffi::c_int
        ...
        .wrapping_add((*token).sni_length as size_t)
        .wrapping_add((*token).ip_addr_length as size_t)
        .wrapping_add((*token).token_length as size_t);
    if required_length > bytes_max {
        ret = PICOQUIC_ERROR_FRAME_BUFFER_TOO_SMALL;
        *consumed = 0 as size_t;
    } else {
        picoformat_64(bytes.offset(byte_index as isize), (*token).time_valid_until);
        byte_index = byte_index.wrapping_add(8 as size_t);
        picoformat_16(bytes.offset(byte_index as isize), (*token).sni_length);
        byte_index = byte_index.wrapping_add(2 as size_t);
        memcpy(
            bytes.offset(byte_index as isize) as *mut ::core::ffi::c_void,
            (*token).sni as *const ::core::ffi::c_void,
            (*token).sni_length as size_t,
        );
        byte_index = byte_index.wrapping_add((*token).sni_length as size_t);
        picoformat_16(
            bytes.offset(byte_index as isize),
            (*token).ip_addr_length as uint16_t,
        );
        byte_index = byte_index.wrapping_add(2 as size_t);
        memcpy(
            bytes.offset(byte_index as isize) as *mut ::core::ffi::c_void,
            (*token).ip_addr as *const ::core::ffi::c_void,
            (*token).ip_addr_length as size_t,
        );
        byte_index = byte_index.wrapping_add((*token).ip_addr_length as size_t);
        picoformat_16(bytes.offset(byte_index as isize), (*token).token_length);
        byte_index = byte_index.wrapping_add(2 as size_t);
        memcpy(
            bytes.offset(byte_index as isize) as *mut ::core::ffi::c_void,
            (*token).token as *const ::core::ffi::c_void,
            (*token).token_length as size_t,
        );
        byte_index = byte_index.wrapping_add((*token).token_length as size_t);
        *consumed = byte_index;
    }
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_deserialize_token(
    mut token: *mut *mut picoquic_stored_token_t,
    mut bytes: *mut uint8_t,
    mut bytes_max: size_t,
    mut consumed: *mut size_t,
) -> ::core::ffi::c_int {
    ...
    let mut token_index: size_t = 0 as size_t;
    let mut sni_length: uint16_t = 0 as uint16_t;
    let mut ip_addr_length: uint8_t = 0 as uint8_t;
    let mut token_length: uint16_t = 0 as uint16_t;
    *consumed = 0 as size_t;
    *token = ::core::ptr::null_mut::<picoquic_stored_token_t>();
    if required_length < bytes_max {
    ...
    if required_length < bytes_max {
        ...
        required_length = required_length.wrapping_add(token_length as size_t);
    }
    if required_length > bytes_max {
        *token = ::core::ptr::null_mut::<picoquic_stored_token_t>();
        ret = PICOQUIC_ERROR_INVALID_TOKEN;
    } else {
        *token = picoquic_format_token(
            time_valid_until,
            bytes.offset(sni_index as isize) as *const ::core::ffi::c_char,
            sni_length,
            bytes.offset(ip_addr_index as isize),
            ip_addr_length,
            bytes.offset(token_index as isize),
            token_length,
        );
        if (*token).is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_store_token(
    ...
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    let mut pp_first_token: *mut *mut picoquic_stored_token_t = &raw mut (*quic).p_first_token;
    let mut current_time: uint64_t = picoquic_get_tls_time(quic);
    if (token_length as ::core::ffi::c_int) < 1 as ::core::ffi::c_int
        || sni.is_null()
        || sni_length as ::core::ffi::c_int == 0 as ::core::ffi::c_int
    {
        ret = PICOQUIC_ERROR_INVALID_TOKEN;
    } else {
        let mut time_valid_until: uint64_t = current_time.wrapping_add(
            (24 as ::core::ffi::c_int as uint64_t)
                .wrapping_mul(3600 as uint64_t)
                .wrapping_mul(1000000 as ::core::ffi::c_int as uint64_t),
        );
        let mut stored: *mut picoquic_stored_token_t = picoquic_format_token(
            time_valid_until,
            sni,
            sni_length,
            ip_addr,
        ...
        if stored.is_null() {
            ...
            let mut next: *mut picoquic_stored_token_t =
                ::core::ptr::null_mut::<picoquic_stored_token_t>();
            let mut pprevious: *mut *mut picoquic_stored_token_t =
                ::core::ptr::null_mut::<*mut picoquic_stored_token_t>();
            next = *pp_first_token;
            (*stored).next_token = next as *mut st_picoquic_stored_token_t;
            *pp_first_token = stored;
            pprevious = &raw mut (*stored).next_token as *mut *mut picoquic_stored_token_t;
            while !next.is_null() {
                if (*next).time_valid_until <= (*stored).time_valid_until
                    && (*next).sni_length as ::core::ffi::c_int == sni_length as ::core::ffi::c_int
                    && (*next).ip_addr_length as ::core::ffi::c_int
                        == ip_addr_length as ::core::ffi::c_int
                    && memcmp(
                    ...
                    && memcmp(
                        (*next).ip_addr as *const ::core::ffi::c_void,
                        ip_addr as *const ::core::ffi::c_void,
                        ip_addr_length as size_t,
                    ) == 0 as ::core::ffi::c_int
                {
                    let mut deleted: *mut picoquic_stored_token_t = next;
                    next = (*next).next_token as *mut picoquic_stored_token_t;
                    *pprevious = next;
                    free(deleted as *mut ::core::ffi::c_void);
                } else {
                    pprevious = &raw mut (*next).next_token as *mut *mut picoquic_stored_token_t;
                    next = (*next).next_token as *mut picoquic_stored_token_t;
                }
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_get_token(
    mut quic: *mut picoquic_quic_t,
    mut sni: *const ::core::ffi::c_char,
    mut sni_length: uint16_t,
    mut ip_addr: *const uint8_t,
    mut ip_addr_length: uint8_t,
    mut token: *mut *mut uint8_t,
    mut token_length: *mut uint16_t,
    mut mark_used: ::core::ffi::c_int,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    let mut current_time: uint64_t = picoquic_get_tls_time(quic);
    let mut p_first_token: *mut picoquic_stored_token_t = (*quic).p_first_token;
    let mut next: *mut picoquic_stored_token_t = p_first_token;
    let mut best_match: *mut picoquic_stored_token_t =
        ::core::ptr::null_mut::<picoquic_stored_token_t>();
    while !next.is_null() {
        ...
        {
            if ip_addr_length as ::core::ffi::c_int > 0 as ::core::ffi::c_int {
                if (*next).ip_addr_length as ::core::ffi::c_int
                    == ip_addr_length as ::core::ffi::c_int
                    && memcmp(
                        (*next).ip_addr as *const ::core::ffi::c_void,
                        ip_addr as *const ::core::ffi::c_void,
            ...
            } else if best_match.is_null()
                || (*next).time_valid_until > (*best_match).time_valid_until
            {
                best_match = next;
            }
        }
    ...
    if best_match.is_null()
        || (*best_match).token_length as ::core::ffi::c_int == 0 as ::core::ffi::c_int
        || {
            *token = malloc((*best_match).token_length as size_t) as *mut uint8_t;
            (*token).is_null()
        }
    {
        *token = ::core::ptr::null_mut::<uint8_t>();
        *token_length = 0 as uint16_t;
        ret = -(1 as ::core::ffi::c_int);
    } else {
        *token_length = (*best_match).token_length;
        memcpy(
            *token as *mut ::core::ffi::c_void,
            (*best_match).token as *mut uint8_t as *const ::core::ffi::c_void,
            (*best_match).token_length as size_t,
        );
        (*best_match).set_was_used(mark_used as ::core::ffi::c_uint as ::core::ffi::c_uint);
    }
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_save_tokens(
    mut quic: *mut picoquic_quic_t,
    mut token_file_name: *const ::core::ffi::c_char,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    let mut F: *mut FILE = ::core::ptr::null_mut::<FILE>();
    let mut first_token: *const picoquic_stored_token_t = (*quic).p_first_token;
    let mut next: *const picoquic_stored_token_t = first_token;
    let mut current_time: uint64_t = picoquic_get_tls_time(quic);
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_load_tokens(
    mut quic: *mut picoquic_quic_t,
    mut token_file_name: *const ::core::ffi::c_char,
) -> ::core::ffi::c_int {
    let mut ret: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
    ...
    let mut F: *mut FILE = ::core::ptr::null_mut::<FILE>();
    let mut previous: *mut picoquic_stored_token_t =
        ::core::ptr::null_mut::<picoquic_stored_token_t>();
    let mut next: *mut picoquic_stored_token_t = ::core::ptr::null_mut::<picoquic_stored_token_t>();
    ...
    while ret == 0 as ::core::ffi::c_int {
        ...
        if storage_size > 2048 as uint32_t || {
            record_size = (storage_size as ::core::ffi::c_ulong)
                .wrapping_add(32 as ::core::ffi::c_ulong) as uint32_t;
            ...
            ) != storage_size as ::core::ffi::c_ulong
            {
                ...
                ret = picoquic_deserialize_token(
                    &raw mut next,
                    &raw mut buffer as *mut uint8_t,
                    storage_size as size_t,
                    &raw mut consumed,
                ...
                if ret == 0 as ::core::ffi::c_int && !next.is_null() {
                    if (*next).time_valid_until < current_time {
                        ...
                        next = ::core::ptr::null_mut::<picoquic_stored_token_t>();
                    } else {
                        (*next).sni =
                            (next as *mut ::core::ffi::c_char).offset(::core::mem::size_of::<
                                picoquic_stored_token_t,
                            >(
                            )
                                as usize
                                as isize);
                        (*next).ip_addr = ((*next).sni as *mut uint8_t)
                            .offset((*next).sni_length as ::core::ffi::c_int as isize)
                            .offset(1 as ::core::ffi::c_int as isize);
                        (*next).token = (*next)
                            .ip_addr
                            .offset((*next).ip_addr_length as ::core::ffi::c_int as isize)
                            .offset(1 as ::core::ffi::c_int as isize)
                            as *mut uint8_t;
                        (*next).next_token = ::core::ptr::null_mut::<st_picoquic_stored_token_t>();
                        if previous.is_null() {
                            *pp_first_token = next;
                        } else {
                            (*previous).next_token = next as *mut st_picoquic_stored_token_t;
                        }
                        previous = next;
                    }
                }
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_free_tokens(
    mut pp_first_token: *mut *mut picoquic_stored_token_t,
) {
    let mut next: *mut picoquic_stored_token_t = ::core::ptr::null_mut::<picoquic_stored_token_t>();
    loop {
        next = *pp_first_token;
        ...
        *pp_first_token = (*next).next_token as *mut picoquic_stored_token_t;
        free(next as *mut ::core::ffi::c_void);
    }
}
