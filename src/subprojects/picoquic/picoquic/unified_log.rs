...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_close_logs(mut quic: *mut picoquic_quic_t) {
    if !(*quic).text_log_fns.is_null() {
        (*(*quic).text_log_fns)
            .log_quic_close
            .expect("non-null function pointer")(quic);
    }
    if !(*quic).bin_log_fns.is_null() {
        (*(*quic).bin_log_fns)
            .log_quic_close
            .expect("non-null function pointer")(quic);
    }
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_quic_pdu(
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_app_message_v(
    mut cnx: *mut picoquic_cnx_t,
    mut fmt: *const ::core::ffi::c_char,
    mut vargs: ::core::ffi::VaList,
) {
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_app_message
            .expect("non-null function pointer")(cnx, fmt, vargs.as_va_list());
    }
    if !(*cnx).f_binlog.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
            .log_app_message
            .expect("non-null function pointer")(cnx, fmt, vargs.as_va_list());
    }
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_app_message(
    mut cnx: *mut picoquic_cnx_t,
    mut fmt: *const ::core::ffi::c_char,
    mut c2rust_args: ...
) {
    if !(*(*cnx).quic).F_log.is_null() {
        ...
        (*(*(*cnx).quic).text_log_fns)
            .log_app_message
            .expect("non-null function pointer")(cnx, fmt, args.as_va_list());
    }
    if !(*cnx).f_binlog.is_null() {
        ...
        (*(*(*cnx).quic).bin_log_fns)
            .log_app_message
            .expect("non-null function pointer")(cnx, fmt, args_0.as_va_list());
    }
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_context_free_app_message(
    mut quic: *mut picoquic_quic_t,
    ...
    if !(*quic).F_log.is_null() {
        ...
        args = c2rust_args.clone();
        (*(*quic).text_log_fns)
            .log_quic_app_message
            .expect("non-null function pointer")(quic, cid, fmt, args.as_va_list());
    }
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_pdu(
    mut cnx: *mut picoquic_cnx_t,
    mut receiving: ::core::ffi::c_int,
    mut current_time: uint64_t,
    mut addr_peer: *const sockaddr,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_packet(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut receiving: ::core::ffi::c_int,
    mut current_time: uint64_t,
    mut ph: *mut st_picoquic_packet_header_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_dropped_packet(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut ph: *mut st_picoquic_packet_header_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_buffered_packet(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    ...
    if picoquic_cnx_is_still_logging(cnx as *mut picoquic_cnx_t) != 0 {
        if !(*(*cnx).quic).F_log.is_null() {
            (*(*(*cnx).quic).text_log_fns)
                .log_buffered_packet
                .expect("non-null function pointer")(cnx, path_x, ptype, current_time);
        }
        if !(*cnx).f_binlog.is_null() {
            (*(*(*cnx).quic).bin_log_fns)
                .log_buffered_packet
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_outgoing_packet(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_packet_lost(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut ptype: picoquic_packet_type_enum,
    mut sequence_number: uint64_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_negotiated_alpn(
    mut cnx: *mut picoquic_cnx_t,
    ...
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_negotiated_alpn
            .expect("non-null function pointer")(
            cnx, is_local, sni, sni_len, alpn, alpn_len, alpn_list, alpn_count,
        );
    }
    if !(*cnx).f_binlog.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_transport_extension(
    mut cnx: *mut picoquic_cnx_t,
    mut is_local: ::core::ffi::c_int,
    ...
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_transport_extension
            .expect("non-null function pointer")(cnx, is_local, param_length, params);
    }
    if !(*cnx).f_binlog.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
            .log_transport_extension
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_tls_ticket(
    mut cnx: *mut picoquic_cnx_t,
    ...
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_picotls_ticket
            .expect("non-null function pointer")(cnx, ticket, ticket_length);
    }
    if !(*cnx).f_binlog.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
            .log_picotls_ticket
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_new_connection(mut cnx: *mut picoquic_cnx_t) {
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_new_connection
            .expect("non-null function pointer")(cnx);
    }
    if !(*(*cnx).quic).bin_log_fns.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
            .log_new_connection
            .expect("non-null function pointer")(cnx);
    }
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_close_connection(mut cnx: *mut picoquic_cnx_t) {
    if !(*(*cnx).quic).F_log.is_null() {
        (*(*(*cnx).quic).text_log_fns)
            .log_close_connection
            .expect("non-null function pointer")(cnx);
    }
    if !(*cnx).f_binlog.is_null() {
        (*(*(*cnx).quic).bin_log_fns)
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_log_cc_dump(
    mut cnx: *mut picoquic_cnx_t,
    mut current_time: uint64_t,
) {
    if picoquic_cnx_is_still_logging(cnx as *mut picoquic_cnx_t) != 0 {
        if !(*(*cnx).quic).F_log.is_null() {
            (*(*(*cnx).quic).text_log_fns)
                .log_cc_dump
                .expect("non-null function pointer")(cnx, current_time);
        }
        if !(*cnx).f_binlog.is_null() {
            (*(*(*cnx).quic).bin_log_fns)
                .log_cc_dump
