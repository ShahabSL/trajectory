...
pub type picoquic_prague_state_t = st_picoquic_prague_state_t;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct st_picoquic_prague_state_t {
    pub alg_state: picoquic_prague_alg_state_t,
    pub alpha_shifted: uint64_t,
    pub alpha: uint64_t,
    pub residual_ack: uint64_t,
    pub ssthresh: uint64_t,
    pub recovery_start: uint64_t,
    pub l4s_update_sent: uint64_t,
    pub l4s_epoch_send: uint64_t,
    pub l4s_epoch_ect1: uint64_t,
    pub l4s_epoch_ce: uint64_t,
...
pub type picoquic_prague_alg_state_t = ::core::ffi::c_uint;
pub const picoquic_prague_alg_congestion_avoidance: picoquic_prague_alg_state_t = 1;
pub const picoquic_prague_alg_slow_start: picoquic_prague_alg_state_t = 0;
...
pub const PICOQUIC_CC_ALGO_NUMBER_PRAGUE: ::core::ffi::c_int = 6 as ::core::ffi::c_int;
pub const NULL: *mut ::core::ffi::c_void =
    ::core::ptr::null::<::core::ffi::c_void>() as *mut ::core::ffi::c_void;
pub const PRAGUE_SHIFT_G: ::core::ffi::c_int = 4 as ::core::ffi::c_int;
unsafe extern "C" fn picoquic_prague_init_reno(
    mut pr_state: *mut picoquic_prague_state_t,
    mut path_x: *mut picoquic_path_t,
) {
    (*pr_state).alg_state = picoquic_prague_alg_slow_start;
    (*pr_state).ssthresh = UINT64_MAX as uint64_t;
    (*pr_state).alpha = 0 as uint64_t;
    (*path_x).cwin = PICOQUIC_CWIN_INITIAL as uint64_t;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_prague_init(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut current_time: uint64_t,
) {
    let mut pr_state: *mut picoquic_prague_state_t =
        malloc(::core::mem::size_of::<picoquic_prague_state_t>() as size_t)
            as *mut picoquic_prague_state_t;
    if !pr_state.is_null() {
        memset(
            pr_state as *mut ::core::ffi::c_void,
            0 as ::core::ffi::c_int,
            ::core::mem::size_of::<picoquic_prague_state_t>() as size_t,
        );
        (*path_x).congestion_alg_state = pr_state as *mut ::core::ffi::c_void;
        picoquic_prague_init_reno(pr_state, path_x);
    } else {
        (*path_x).congestion_alg_state = NULL;
    };
}
unsafe extern "C" fn picoquic_prague_get_pkt_ctx(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
...
unsafe extern "C" fn picoquic_prague_reset_l3s(
    mut cnx: *mut picoquic_cnx_t,
    mut pr_state: *mut picoquic_prague_state_t,
    mut path_x: *mut picoquic_path_t,
) {
    let mut pkt_ctx: *mut picoquic_packet_context_t = picoquic_prague_get_pkt_ctx(cnx, path_x);
    ...
    (*pr_state).alpha = 0 as uint64_t;
    (*pr_state).alpha_shifted = 0 as uint64_t;
}
unsafe extern "C" fn picoquic_prague_reset(
    mut cnx: *mut picoquic_cnx_t,
    mut pr_state: *mut picoquic_prague_state_t,
    mut path_x: *mut picoquic_path_t,
) {
...
unsafe extern "C" fn picoquic_prague_enter_recovery(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    ...
    (*pr_state).ssthresh = (*path_x).cwin.wrapping_div(2 as uint64_t);
    if (*pr_state).ssthresh < PICOQUIC_CWIN_MINIMUM as uint64_t {
        (*pr_state).ssthresh = PICOQUIC_CWIN_MINIMUM as uint64_t;
    }
    ...
    (*pr_state).residual_ack = 0 as uint64_t;
    picoquic_prague_reset_l3s(cnx, pr_state, path_x);
}
unsafe extern "C" fn picoquic_prague_update_alpha(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut pr_state: *mut picoquic_prague_state_t,
    mut nb_bytes_acknowledged: uint64_t,
    mut current_time: uint64_t,
) {
    let mut pkt_ctx: *mut picoquic_packet_context_t = picoquic_prague_get_pkt_ctx(cnx, path_x);
    let mut update_sent: uint64_t = (*pkt_ctx).latest_time_acknowledged;
    if (*pkt_ctx).highest_acknowledged != UINT64_MAX as uint64_t
        && (*pkt_ctx).highest_acknowledged > (*pr_state).l4s_epoch_send
    {
        let mut frac: uint64_t = 0 as uint64_t;
        let mut is_suspect: ::core::ffi::c_int = 0 as ::core::ffi::c_int;
        (*pr_state).l4s_epoch_send = (*pkt_ctx).send_sequence;
        let mut delta_ect1: uint64_t = (*pkt_ctx)
            .ecn_ect1_total_remote
            .wrapping_sub((*pr_state).l4s_epoch_ect1);
        let mut delta_ce: uint64_t = (*pkt_ctx)
            .ecn_ce_total_remote
            .wrapping_sub((*pr_state).l4s_epoch_ce);
        if delta_ce > 0 as uint64_t {
            frac = delta_ce
                .wrapping_mul(1024 as uint64_t)
                .wrapping_div(delta_ce.wrapping_add(delta_ect1));
        } else {
            frac = 0 as uint64_t;
        }
        if (*pr_state).l4s_update_sent != 0 as uint64_t
            && frac > 512 as uint64_t
            && (*pr_state).alpha < 128 as uint64_t
            && update_sent.wrapping_sub((*pr_state).l4s_update_sent) > (*path_x).smoothed_rtt
        {
            is_suspect = 1 as ::core::ffi::c_int;
            frac = 128 as uint64_t;
        }
        (*pr_state).l4s_update_sent = update_sent;
        if delta_ce > 0 as uint64_t || delta_ect1 > 0 as uint64_t {
            if frac > (*pr_state).alpha && (frac > 512 as uint64_t || is_suspect != 0) {
                (*pr_state).alpha = frac;
                (*pr_state).alpha_shifted = frac << PRAGUE_SHIFT_G;
            } else {
                let mut delta_frac: int64_t = frac.wrapping_sub((*pr_state).alpha) as int64_t;
                (*pr_state).alpha_shifted = (*pr_state)
                    .alpha_shifted
                    .wrapping_add(delta_frac as uint64_t);
        ...
        (*pr_state).l4s_epoch_ce = (*pkt_ctx).ecn_ce_total_remote;
        if delta_ce > 0 as uint64_t {
            if (*pr_state).alpha > 512 as uint64_t {
                picoquic_prague_enter_recovery(
                    cnx,
                    path_x,
                ...
                let mut reduction: uint64_t = (*path_x)
                    .cwin
                    .wrapping_mul((*pr_state).alpha)
                    .wrapping_div(2048 as uint64_t);
                (*pr_state).ssthresh = (*path_x).cwin.wrapping_sub(reduction);
                if (*pr_state).ssthresh < PICOQUIC_CWIN_MINIMUM as uint64_t {
                    (*pr_state).ssthresh = PICOQUIC_CWIN_MINIMUM as uint64_t;
                }
                let mut old_cwin: uint64_t = (*path_x).cwin;
                (*path_x).cwin = (*pr_state).ssthresh;
                (*pr_state).alg_state = picoquic_prague_alg_congestion_avoidance;
                picoquic_log_app_message(
                    cnx as *mut picoquic_cnx_t,
                    b"Prague alpha: %lu, cwin, was % lu is now % lu\n\0".as_ptr()
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_prague_notify(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut notification: picoquic_congestion_notification_t,
    ...
    let mut pr_state: *mut picoquic_prague_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_prague_state_t;
    if !pr_state.is_null() {
        match notification as ::core::ffi::c_uint {
            0 => {
                if (*pr_state).alg_state as ::core::ffi::c_uint
                    == picoquic_prague_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
                    && (*pr_state).ssthresh == UINT64_MAX as uint64_t
                {
                ...
                match (*pr_state).alg_state as ::core::ffi::c_uint {
                    0 => {
                        if (*path_x).smoothed_rtt as ::core::ffi::c_ulonglong
                            <= PICOQUIC_TARGET_RENO_RTT
                        {
                            (*path_x).cwin = (*path_x).cwin.wrapping_add(
                                (*ack_state)
                                    .nb_bytes_acknowledged
                                    .wrapping_mul(
                                        (1024 as uint64_t).wrapping_sub((*pr_state).alpha),
                                    )
                                    .wrapping_div(1024 as uint64_t),
                            );
                        } else {
                            let mut delta: uint64_t = (*ack_state).nb_bytes_acknowledged;
                            delta = delta.wrapping_mul((*path_x).smoothed_rtt);
                            delta = delta
                                .wrapping_mul((1024 as uint64_t).wrapping_sub((*pr_state).alpha));
                            delta = (delta as ::core::ffi::c_ulonglong)
                                .wrapping_div(PICOQUIC_TARGET_RENO_RTT)
                                as uint64_t as uint64_t;
                            delta = delta.wrapping_div(1024 as uint64_t);
                            (*path_x).cwin = (*path_x).cwin.wrapping_add(delta);
                        }
                    ...
                    1 | _ => {
                        let mut complete_delta: uint64_t = (*ack_state)
                            .nb_bytes_acknowledged
                            .wrapping_mul((*path_x).send_mtu as uint64_t)
                            .wrapping_add((*pr_state).residual_ack);
                        (*pr_state).residual_ack = complete_delta.wrapping_rem((*path_x).cwin);
                        let mut delta_0: uint64_t = complete_delta.wrapping_div((*path_x).cwin);
                        delta_0 = delta_0
                            .wrapping_mul((1024 as uint64_t).wrapping_sub((*pr_state).alpha))
                            .wrapping_div(1024 as uint64_t);
                        (*path_x).cwin = (*path_x).cwin.wrapping_add(delta_0);
                    }
                }
            }
            5 => {
                if (*pr_state).alg_state as ::core::ffi::c_uint
                    ...
                    && (*pr_state).ssthresh == UINT64_MAX as uint64_t
                {
                    if (*path_x).cwin > (*path_x).send_mtu as uint64_t {
                        (*path_x).cwin = ((*path_x).cwin as ::core::ffi::c_ulong)
                            .wrapping_sub((*path_x).send_mtu as ::core::ffi::c_ulong)
                            as uint64_t as uint64_t;
                    }
            ...
            1 | 2 => {
                if current_time.wrapping_sub((*pr_state).recovery_start) > (*path_x).smoothed_rtt {
                    picoquic_prague_enter_recovery(
                        cnx,
                        path_x,
                        notification,
            ...
            3 => {
                if current_time.wrapping_sub((*pr_state).recovery_start) < (*path_x).smoothed_rtt {
                    if (*path_x).cwin < (2 as uint64_t).wrapping_mul((*pr_state).ssthresh) {
                        (*path_x).cwin = (2 as uint64_t).wrapping_mul((*pr_state).ssthresh);
                        (*pr_state).alg_state = picoquic_prague_alg_congestion_avoidance;
                    }
                }
            }
            4 => {
                    ...
                    && (*pr_state).ssthresh == UINT64_MAX as uint64_t
                {
                    if (*path_x).rtt_min as ::core::ffi::c_ulonglong > PICOQUIC_TARGET_RENO_RTT {
                        let mut min_win_0: uint64_t = 0;
                        if (*path_x).rtt_min as ::core::ffi::c_ulonglong
                            > PICOQUIC_TARGET_SATELLITE_RTT
                        {
                            min_win_0 = (PICOQUIC_CWIN_INITIAL as ::core::ffi::c_double
                        ...
                        if min_win_0 > (*path_x).cwin {
                            (*path_x).cwin = min_win_0;
                        }
                    }
                    if picoquic_hystart_test(
                        &raw mut (*pr_state).rtt_filter,
    ...
    picoquic_update_pacing_data(
        cnx,
        path_x,
        ((*pr_state).alg_state as ::core::ffi::c_uint
            == picoquic_prague_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
            && (*pr_state).ssthresh == UINT64_MAX as uint64_t) as ::core::ffi::c_int,
    );
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_prague_delete(mut path_x: *mut picoquic_path_t) {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_prague_observe(
    mut path_x: *mut picoquic_path_t,
    ...
    let mut pr_state: *mut picoquic_prague_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_prague_state_t;
    *cc_state = (*pr_state).alg_state as uint64_t;
    *cc_param = if (*pr_state).ssthresh == UINT64_MAX as uint64_t {
        0 as uint64_t
    } else {
        (*pr_state).ssthresh
    };
}
pub const PICOQUIC_PRAGUE_ID: [::core::ffi::c_char; 7] =
    unsafe { ::core::mem::transmute::<[u8; 7], [::core::ffi::c_char; 7]>(*b"prague\0") };
#[no_mangle]
pub static mut picoquic_prague_algorithm_struct: picoquic_congestion_algorithm_t = unsafe {
    st_picoquic_congestion_algorithm_t {
        congestion_algorithm_id: PICOQUIC_PRAGUE_ID.as_ptr(),
        congestion_algorithm_number: PICOQUIC_CC_ALGO_NUMBER_PRAGUE as uint8_t,
        alg_init: Some(
        ...
        alg_delete: Some(
            picoquic_prague_delete as unsafe extern "C" fn(*mut picoquic_path_t) -> (),
        ),
        alg_observe: Some(
...
#[no_mangle]
pub static mut picoquic_prague_algorithm: *mut picoquic_congestion_algorithm_t =
    unsafe { &raw const picoquic_prague_algorithm_struct as *mut picoquic_congestion_algorithm_t };
