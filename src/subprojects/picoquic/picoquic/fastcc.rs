...
extern "C" {
    ...
    fn picoquic_filter_rtt_min_max(rtt_track: *mut picoquic_min_max_rtt_t, rtt: uint64_t);
...
pub type picoquic_fastcc_state_t = st_picoquic_fastcc_state_t;
#[derive(Copy, Clone, BitfieldStruct)]
#[repr(C)]
pub struct st_picoquic_fastcc_state_t {
    pub alg_state: picoquic_fastcc_alg_state_t,
    pub end_of_freeze: uint64_t,
    pub last_ack_time: uint64_t,
    pub ack_interval: uint64_t,
    pub nb_bytes_ack: uint64_t,
    pub nb_bytes_ack_since_rtt: uint64_t,
    pub end_of_epoch: uint64_t,
    pub recovery_sequence: uint64_t,
    pub rtt_min: uint64_t,
    pub delay_threshold: uint64_t,
    pub rolling_rtt_min: uint64_t,
    pub last_rtt_min: [uint64_t; 6],
    pub nb_cc_events: ::core::ffi::c_int,
    #[bitfield(
        name = "last_freeze_was_timeout",
        ty = "::core::ffi::c_uint",
        bits = "0..=0"
    )]
    #[bitfield(
        name = "last_freeze_was_not_delay",
        ty = "::core::ffi::c_uint",
        bits = "1..=1"
    )]
    #[bitfield(
        name = "rtt_min_is_trusted",
        ty = "::core::ffi::c_uint",
        bits = "2..=2"
    )]
    pub last_freeze_was_timeout_last_freeze_was_not_delay_rtt_min_is_trusted: [u8; 1],
    #[bitfield(padding)]
    pub c2rust_padding: [u8; 3],
...
pub type picoquic_fastcc_alg_state_t = ::core::ffi::c_uint;
pub const picoquic_fastcc_freeze: picoquic_fastcc_alg_state_t = 2;
pub const picoquic_fastcc_eval: picoquic_fastcc_alg_state_t = 1;
pub const picoquic_fastcc_initial: picoquic_fastcc_alg_state_t = 0;
...
pub const FASTCC_REPEAT_THRESHOLD: ::core::ffi::c_int = 4 as ::core::ffi::c_int;
pub const FASTCC_BETA: ::core::ffi::c_double = 0.125f64;
pub const FASTCC_EVAL_ALPHA: ::core::ffi::c_double = 0.25f64;
pub const FASTCC_DELAY_THRESHOLD_MAX: ::core::ffi::c_int = 25000 as ::core::ffi::c_int;
pub const FASTCC_NB_PERIOD: ::core::ffi::c_int = 6 as ::core::ffi::c_int;
pub const FASTCC_PERIOD: ::core::ffi::c_int = 1000000 as ::core::ffi::c_int;
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_delay_threshold(mut rtt_min: uint64_t) -> uint64_t {
    let mut delay: uint64_t = rtt_min.wrapping_div(8 as uint64_t);
    if delay > FASTCC_DELAY_THRESHOLD_MAX as uint64_t {
        delay = FASTCC_DELAY_THRESHOLD_MAX as uint64_t;
    }
    return delay;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_reset(
    mut fastcc_state: *mut picoquic_fastcc_state_t,
    mut path_x: *mut picoquic_path_t,
    mut current_time: uint64_t,
) {
    memset(
        fastcc_state as *mut ::core::ffi::c_void,
        0 as ::core::ffi::c_int,
        ::core::mem::size_of::<picoquic_fastcc_state_t>() as size_t,
    );
    ...
    (*fastcc_state).end_of_epoch = current_time.wrapping_add(FASTCC_PERIOD as uint64_t);
    (*path_x).cwin = PICOQUIC_CWIN_INITIAL as uint64_t;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_seed_cwin(
    mut fastcc_state: *mut picoquic_fastcc_state_t,
    mut path_x: *mut picoquic_path_t,
    mut bytes_in_flight: uint64_t,
) {
    if (*fastcc_state).alg_state as ::core::ffi::c_uint
        == picoquic_fastcc_initial as ::core::ffi::c_int as ::core::ffi::c_uint
    {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_init(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut current_time: uint64_t,
) {
    let mut fastcc_state: *mut picoquic_fastcc_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_fastcc_state_t;
    if fastcc_state.is_null() {
        fastcc_state = malloc(::core::mem::size_of::<picoquic_fastcc_state_t>() as size_t)
            as *mut picoquic_fastcc_state_t;
    }
    if !fastcc_state.is_null() {
        memset(
            fastcc_state as *mut ::core::ffi::c_void,
    ...
    (*path_x).congestion_alg_state = fastcc_state as *mut ::core::ffi::c_void;
}
unsafe extern "C" fn fastcc_notify_congestion(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut fastcc_state: *mut picoquic_fastcc_state_t,
    mut current_time: uint64_t,
    mut is_delay: ::core::ffi::c_int,
    mut is_timeout: ::core::ffi::c_int,
) {
    if (*fastcc_state).alg_state as ::core::ffi::c_uint
        == picoquic_fastcc_freeze as ::core::ffi::c_int as ::core::ffi::c_uint
        && (is_timeout == 0 || (*fastcc_state).last_freeze_was_timeout() == 0)
        && (is_delay == 0 || (*fastcc_state).last_freeze_was_not_delay() == 0)
    {
        return;
    }
    (*fastcc_state).set_last_freeze_was_not_delay(
        (is_delay == 0) as ::core::ffi::c_int as ::core::ffi::c_uint as ::core::ffi::c_uint,
    );
    (*fastcc_state)
        .set_last_freeze_was_timeout(is_timeout as ::core::ffi::c_uint as ::core::ffi::c_uint);
    (*fastcc_state).alg_state = picoquic_fastcc_freeze;
    (*fastcc_state).end_of_freeze = current_time.wrapping_add((*fastcc_state).rtt_min);
    (*fastcc_state).recovery_sequence = picoquic_cc_get_sequence_number(cnx, path_x);
    (*fastcc_state).nb_cc_events = 0 as ::core::ffi::c_int;
    if is_delay != 0 {
        (*path_x).cwin = (*path_x)
            .cwin
            .wrapping_sub((FASTCC_BETA * (*path_x).cwin as ::core::ffi::c_double) as uint64_t);
    } else {
    ...
    picoquic_update_pacing_data(cnx, path_x, 0 as ::core::ffi::c_int);
    (*path_x).set_is_ssthresh_initialized(1 as ::core::ffi::c_uint as ::core::ffi::c_uint);
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_notify(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    ...
    if !fastcc_state.is_null() {
        ...
        {
            if (*fastcc_state).last_freeze_was_timeout() != 0 {
                (*fastcc_state).alg_state = picoquic_fastcc_initial;
            } else {
                (*fastcc_state).alg_state = picoquic_fastcc_eval;
            }
            (*fastcc_state)
                .set_last_freeze_was_not_delay(0 as ::core::ffi::c_uint as ::core::ffi::c_uint);
            (*fastcc_state)
                .set_last_freeze_was_timeout(0 as ::core::ffi::c_uint as ::core::ffi::c_uint);
            (*fastcc_state).nb_cc_events = 0 as ::core::ffi::c_int;
            (*fastcc_state).nb_bytes_ack_since_rtt = 0 as uint64_t;
        }
        match notification as ::core::ffi::c_uint {
            0 => {
                if (*fastcc_state).alg_state as ::core::ffi::c_uint
                    != picoquic_fastcc_freeze as ::core::ffi::c_int as ::core::ffi::c_uint
                {
                    (*fastcc_state).nb_bytes_ack_since_rtt = (*fastcc_state)
            ...
            1 | 2 => {
                if picoquic_hystart_loss_test(
                    &raw mut (*fastcc_state).rtt_filter,
                    notification,
            ...
            3 => {
                if (*fastcc_state).nb_cc_events > 0 as ::core::ffi::c_int {
                    (*fastcc_state).nb_cc_events -= 1;
                }
            }
            4 => {
                let mut delta_rtt: uint64_t = 0 as uint64_t;
                picoquic_filter_rtt_min_max(
                    &raw mut (*fastcc_state).rtt_filter,
                    (*ack_state).rtt_measurement,
                );
                if (*fastcc_state).rtt_filter.is_init != 0 {
                    if current_time > (*fastcc_state).end_of_epoch {
                        (*fastcc_state).rtt_min = UINT64_MAX as uint64_t;
                        let mut i: ::core::ffi::c_int = FASTCC_NB_PERIOD - 1 as ::core::ffi::c_int;
                        while i > 0 as ::core::ffi::c_int {
                            (*fastcc_state).last_rtt_min[i as usize] = (*fastcc_state).last_rtt_min
                                [(i - 1 as ::core::ffi::c_int) as usize];
                            if (*fastcc_state).last_rtt_min[i as usize] > 0 as uint64_t
                                && (*fastcc_state).last_rtt_min[i as usize]
                                    < (*fastcc_state).rtt_min
                            {
                                (*fastcc_state).rtt_min = (*fastcc_state).last_rtt_min[i as usize];
                            }
                            i -= 1;
                        }
                        (*fastcc_state).delay_threshold =
                            picoquic_fastcc_delay_threshold((*fastcc_state).rtt_min);
                        (*fastcc_state).last_rtt_min[0 as ::core::ffi::c_int as usize] =
                            (*fastcc_state).rolling_rtt_min;
                        (*fastcc_state).rolling_rtt_min = (*fastcc_state).rtt_filter.sample_max;
                        (*fastcc_state).end_of_epoch =
                            current_time.wrapping_add(FASTCC_PERIOD as uint64_t);
                    } else if (*fastcc_state).rtt_filter.sample_max
                        < (*fastcc_state).rolling_rtt_min
                        || (*fastcc_state).rolling_rtt_min == 0 as uint64_t
                    {
                        (*fastcc_state).rolling_rtt_min = (*fastcc_state).rtt_filter.sample_max;
                    ...
                    != picoquic_fastcc_freeze as ::core::ffi::c_int as ::core::ffi::c_uint
                {
                    if (*ack_state).rtt_measurement < (*fastcc_state).rtt_min {
                        (*fastcc_state).delay_threshold =
                            picoquic_fastcc_delay_threshold((*fastcc_state).rtt_min);
                    } else if (*fastcc_state).rtt_min_is_trusted() != 0 {
                        delta_rtt = (*ack_state)
                            .rtt_measurement
                            .wrapping_sub((*fastcc_state).rtt_min);
                    } else {
                        (*fastcc_state).rtt_min = (*ack_state).rtt_measurement;
                        (*fastcc_state).rolling_rtt_min = (*ack_state).rtt_measurement;
                        (*fastcc_state).set_rtt_min_is_trusted(
                            1 as ::core::ffi::c_uint as ::core::ffi::c_uint,
                        );
                        delta_rtt = 0 as uint64_t;
                    }
                    if delta_rtt < (*fastcc_state).delay_threshold {
                        let mut alpha: ::core::ffi::c_double = 1.0f64;
                        (*fastcc_state).nb_cc_events = 0 as ::core::ffi::c_int;
                        if (*fastcc_state).alg_state as ::core::ffi::c_uint
                            != picoquic_fastcc_initial as ::core::ffi::c_int as ::core::ffi::c_uint
                        {
                            alpha -= delta_rtt as ::core::ffi::c_double
                                / (*fastcc_state).delay_threshold as ::core::ffi::c_double;
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_delete(mut path_x: *mut picoquic_path_t) {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_fastcc_observe(
    mut path_x: *mut picoquic_path_t,
    ...
    let mut fastcc_state: *mut picoquic_fastcc_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_fastcc_state_t;
    *cc_state = (*fastcc_state).alg_state as uint64_t;
    *cc_param = (*fastcc_state).rolling_rtt_min;
}
pub const picoquic_fastcc_ID: [::core::ffi::c_char; 5] =
    unsafe { ::core::mem::transmute::<[u8; 5], [::core::ffi::c_char; 5]>(*b"fast\0") };
#[no_mangle]
pub static mut picoquic_fastcc_algorithm_struct: picoquic_congestion_algorithm_t = unsafe {
    st_picoquic_congestion_algorithm_t {
        congestion_algorithm_id: picoquic_fastcc_ID.as_ptr(),
        congestion_algorithm_number: PICOQUIC_CC_ALGO_NUMBER_FAST as uint8_t,
        alg_init: Some(
        ...
        alg_delete: Some(
            picoquic_fastcc_delete as unsafe extern "C" fn(*mut picoquic_path_t) -> (),
        ),
        alg_observe: Some(
...
#[no_mangle]
pub static mut picoquic_fastcc_algorithm: *mut picoquic_congestion_algorithm_t =
    unsafe { &raw const picoquic_fastcc_algorithm_struct as *mut picoquic_congestion_algorithm_t };
