...
extern "C" {
    ...
    fn picoquic_issue_path_quality_update(
...
pub const PICOQUIC_LARGE_RETRANSMIT_TIMER: ::core::ffi::c_ulonglong =
    2000000 as ::core::ffi::c_ulonglong;
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_current_retransmit_timer(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
) -> uint64_t {
    let mut rto: uint64_t = (*path_x).retransmit_timer;
    if (*path_x).nb_retransmit > 0 as uint64_t {
        if (*path_x).nb_retransmit < 3 as uint64_t {
            rto <<= (*path_x).nb_retransmit;
        } else {
            let mut n1: uint64_t = (*path_x).nb_retransmit.wrapping_sub(2 as uint64_t);
            if n1 > 18 as uint64_t {
                n1 = 18 as uint64_t;
            }
            rto <<= (2 as uint64_t).wrapping_add(n1.wrapping_div(4 as uint64_t));
            n1 &= 3 as uint64_t;
            rto = rto.wrapping_add(n1.wrapping_mul(rto) >> 2 as ::core::ffi::c_int);
        }
        if (*cnx).idle_timeout > 15 as uint64_t {
            if rto > (*cnx).idle_timeout >> 4 as ::core::ffi::c_int {
                rto = (*cnx).idle_timeout >> 4 as ::core::ffi::c_int;
            }
        }
    }
    ...
    {
        if PICOQUIC_MICROSEC_HANDSHAKE_MAX.wrapping_div(1000 as ::core::ffi::c_ulonglong)
        ...
        } else if rto as ::core::ffi::c_ulonglong > PICOQUIC_INITIAL_MAX_RETRANSMIT_TIMER {
            rto = PICOQUIC_INITIAL_MAX_RETRANSMIT_TIMER as uint64_t;
        }
    } else if rto as ::core::ffi::c_ulonglong > PICOQUIC_LARGE_RETRANSMIT_TIMER {
        let mut alt_rto: uint64_t = PICOQUIC_LARGE_RETRANSMIT_TIMER as uint64_t;
        if (*path_x).rtt_min as ::core::ffi::c_ulonglong > PICOQUIC_TARGET_SATELLITE_RTT {
            alt_rto = (*path_x).smoothed_rtt.wrapping_mul(3 as uint64_t) >> 1 as ::core::ffi::c_int;
        }
        if alt_rto < rto {
            rto = alt_rto;
        }
...
unsafe extern "C" fn picoquic_validate_bdp_seed(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    ...
    if path_x == *(*cnx).path.offset(0 as ::core::ffi::c_int as isize)
        && (*cnx).seed_cwin != 0 as uint64_t
        && (*cnx).cwin_notified_from_seed() == 0
    {
        let mut rtt_margin: uint64_t = rtt_sample.wrapping_div(4 as uint64_t);
        if (*cnx).seed_rtt_min >= rtt_sample.wrapping_sub(rtt_margin)
            && (*cnx).seed_rtt_min <= rtt_sample.wrapping_add(rtt_margin)
        {
            ...
            if ip_addr_length as ::core::ffi::c_int
                == (*cnx).seed_ip_addr_length as ::core::ffi::c_int
                && memcmp(
                    ip_addr as *const ::core::ffi::c_void,
                    &raw mut (*cnx).seed_ip_addr as *mut uint8_t as *const ::core::ffi::c_void,
                ...
                ) == 0 as ::core::ffi::c_int
            {
                ...
                (*cnx).set_cwin_notified_from_seed(1 as ::core::ffi::c_uint as ::core::ffi::c_uint);
                (*(*cnx).congestion_alg)
                    .alg_notify
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_path_rtt_one_way(
    mut cnx: *mut picoquic_cnx_t,
    mut old_path: *mut picoquic_path_t,
    mut path_x: *mut picoquic_path_t,
    mut send_time: uint64_t,
    mut current_time: uint64_t,
    mut ack_delay: uint64_t,
    mut time_stamp: uint64_t,
) {
    if time_stamp != 0 as uint64_t {
        if (*cnx).phase_delay == INT64_MAX as int64_t {
            (*cnx).phase_delay = (*old_path).rtt_sample.wrapping_div(2 as uint64_t) as int64_t;
            if (*cnx).client_mode() == 0 {
                (*cnx).phase_delay = -(*cnx).phase_delay;
            }
        }
        let mut time_stamp_local: int64_t = time_stamp
            .wrapping_sub(ack_delay)
            .wrapping_add((*cnx).start_time)
            .wrapping_add((*cnx).phase_delay as uint64_t)
            as int64_t;
        let mut is_time_stamp_valid: ::core::ffi::c_int = 1 as ::core::ffi::c_int;
        if time_stamp_local < 0 as int64_t || (time_stamp_local as uint64_t) < send_time {
            let mut min_phase: int64_t = send_time
                .wrapping_sub(time_stamp)
            ...
            time_stamp_local = time_stamp
                .wrapping_sub(ack_delay)
                .wrapping_add((*cnx).start_time)
                .wrapping_add(min_phase as uint64_t) as int64_t;
            if time_stamp_local > 0 as int64_t && time_stamp_local as uint64_t <= current_time {
                (*cnx).phase_delay = min_phase;
            } else {
                is_time_stamp_valid = 0 as ::core::ffi::c_int;
            }
        } else if time_stamp_local as uint64_t > current_time {
            let mut max_phase: int64_t = current_time
                .wrapping_sub(time_stamp)
                .wrapping_add(ack_delay)
                .wrapping_sub((*cnx).start_time)
                as int64_t;
            time_stamp_local = time_stamp
                .wrapping_sub(ack_delay)
                .wrapping_add((*cnx).start_time)
                .wrapping_add(max_phase as uint64_t) as int64_t;
            if time_stamp_local > 0 as int64_t && time_stamp_local as uint64_t >= send_time {
        ...
        if is_time_stamp_valid != 0 {
            (*old_path).one_way_delay_sample =
                (time_stamp_local as uint64_t).wrapping_sub(send_time);
        } else {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_path_rtt(
    mut cnx: *mut picoquic_cnx_t,
    mut old_path: *mut picoquic_path_t,
    mut path_x: *mut picoquic_path_t,
    ...
    if !old_path.is_null()
        && ((*old_path).rtt_is_initialized() == 0 || epoch >= 0 as ::core::ffi::c_int)
    {
        let mut rtt_estimate: uint64_t = 0 as uint64_t;
        let mut is_first: ::core::ffi::c_int =
            ((*old_path).rtt_is_initialized() == 0) as ::core::ffi::c_int;
        let mut pkt_ctx: *mut picoquic_packet_context_t =
            ::core::ptr::null_mut::<picoquic_packet_context_t>();
        if (*cnx).is_multipath_enabled() != 0 {
            pkt_ctx = &raw mut (*old_path).pkt_ctx;
        } else if epoch == picoquic_epoch_1rtt as ::core::ffi::c_int
        ...
        if current_time > send_time {
            rtt_estimate = current_time.wrapping_sub(send_time);
            if is_first == 0
                && ack_delay > 0 as uint64_t
                && (*cnx).cnx_state as ::core::ffi::c_uint
                    >= picoquic_state_ready as ::core::ffi::c_int as ::core::ffi::c_uint
            {
                if ack_delay > (*cnx).local_parameters.max_ack_delay as uint64_t {
                    ack_delay = (*cnx).local_parameters.max_ack_delay as uint64_t;
                }
                if (*old_path).rtt_min.wrapping_add(ack_delay) < rtt_estimate {
                    rtt_estimate = rtt_estimate.wrapping_sub(ack_delay);
                }
            }
        }
        (*old_path).rtt_sample = rtt_estimate;
        (*old_path).nb_rtt_estimate_in_period = (*old_path)
            .nb_rtt_estimate_in_period
            .wrapping_add(1 as uint64_t);
        (*old_path).sum_rtt_estimate_in_period = (*old_path)
            .sum_rtt_estimate_in_period
            .wrapping_add(rtt_estimate);
        if (*old_path).nb_rtt_estimate_in_period == 1 as uint64_t {
        ...
        if time_stamp > 0 as uint64_t {
            picoquic_update_path_rtt_one_way(
                cnx,
                old_path,
                path_x,
                send_time,
        ...
        if pkt_ctx.is_null()
            || (*pkt_ctx).highest_acknowledged > (*old_path).rtt_packet_previous_period
            || (*old_path)
                .rtt_time_previous_period
                .wrapping_add(rtt_estimate.wrapping_div(4 as uint64_t))
                > current_time
        {
            ...
            if (*old_path).nb_rtt_estimate_in_period > 1 as uint64_t {
                rtt_estimate = (*old_path)
                    .sum_rtt_estimate_in_period
                    .wrapping_div((*old_path).nb_rtt_estimate_in_period);
            }
            if is_first != 0 {
                (*old_path).smoothed_rtt = rtt_estimate;
                (*old_path).rtt_variant = rtt_estimate.wrapping_div(2 as uint64_t);
                (*old_path).rtt_min = (*old_path).min_rtt_estimate_in_period;
                (*old_path).set_rtt_is_initialized(1 as ::core::ffi::c_uint as ::core::ffi::c_uint);
            } else {
                let mut rtt_var_sample: uint64_t = 0 as uint64_t;
                if (*old_path).smoothed_rtt > (*old_path).max_rtt_estimate_in_period {
                    rtt_var_sample = (*old_path)
                        .smoothed_rtt
                        .wrapping_sub((*old_path).min_rtt_estimate_in_period);
                } else if (*old_path).smoothed_rtt < (*old_path).min_rtt_estimate_in_period {
                    rtt_var_sample = (*old_path)
                        .max_rtt_estimate_in_period
                        .wrapping_sub((*old_path).smoothed_rtt);
                } else {
                    let mut rtt_var_sample_min: uint64_t = (*old_path)
                        .smoothed_rtt
                        .wrapping_sub((*old_path).min_rtt_estimate_in_period);
                    rtt_var_sample = (*old_path)
                        .max_rtt_estimate_in_period
                ...
                (*old_path).rtt_variant = (3 as uint64_t)
                    .wrapping_mul((*old_path).rtt_variant)
                    .wrapping_add(rtt_var_sample)
                    .wrapping_div(4 as uint64_t);
                (*old_path).smoothed_rtt = (7 as uint64_t)
                    .wrapping_mul((*old_path).smoothed_rtt)
                    .wrapping_add(rtt_estimate)
                    .wrapping_div(8 as uint64_t);
            }
            (*old_path).retransmit_timer = (*old_path)
                .smoothed_rtt
                .wrapping_add((3 as uint64_t).wrapping_mul((*old_path).rtt_variant))
                .wrapping_add((*cnx).remote_parameters.max_ack_delay as uint64_t);
            if old_path == *(*cnx).path.offset(0 as ::core::ffi::c_int as isize)
                && (*cnx).cnx_state as ::core::ffi::c_uint
                    >= picoquic_state_ready as ::core::ffi::c_int as ::core::ffi::c_uint
            {
                (*cnx).set_is_ack_frequency_updated(
                    (*cnx).is_ack_frequency_negotiated() as ::core::ffi::c_uint
                );
            ...
            (*old_path).nb_rtt_estimate_in_period = 0 as uint64_t;
            (*old_path).sum_rtt_estimate_in_period = 0 as uint64_t;
            (*old_path).max_rtt_estimate_in_period = 0 as uint64_t;
            (*old_path).min_rtt_estimate_in_period = UINT64_MAX as uint64_t;
        }
        if !(*cnx).congestion_alg.is_null() {
            ...
            ack_state.one_way_delay = if (*cnx).is_time_stamp_enabled() as ::core::ffi::c_int != 0 {
                (*old_path).one_way_delay_sample
            } else {
                0 as uint64_t
            };
        ...
        picoquic_issue_path_quality_update(cnx, old_path);
    }
}
