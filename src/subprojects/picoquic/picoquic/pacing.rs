...
extern "C" {
    ...
    fn picoquic_refresh_path_quality_thresholds(path_x: *mut picoquic_path_t);
}
...
pub const PICOQUIC_MAX_BANDWIDTH_TIME_INTERVAL_MAX: ::core::ffi::c_int =
    15000 as ::core::ffi::c_int;
pub const NULL: *mut ::core::ffi::c_void =
    ::core::ptr::null::<::core::ffi::c_void>() as *mut ::core::ffi::c_void;
#[no_mangle]
pub unsafe extern "C" fn picoquic_pacing_init(
    mut pacing: *mut picoquic_pacing_t,
    mut current_time: uint64_t,
) {
    (*pacing).evaluation_time = current_time;
    (*pacing).bucket_nanosec = 16 as int64_t;
    (*pacing).bucket_max = 16 as int64_t;
    (*pacing).packet_time_nanosec = 1 as int64_t;
    (*pacing).packet_time_microsec = 1 as uint64_t;
}
unsafe extern "C" fn picoquic_update_pacing_bucket(
    mut pacing: *mut picoquic_pacing_t,
    mut current_time: uint64_t,
) {
    ...
    if current_time > (*pacing).evaluation_time {
        (*pacing).bucket_nanosec = ((*pacing).bucket_nanosec as uint64_t).wrapping_add(
            current_time
                .wrapping_sub((*pacing).evaluation_time)
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_is_pacing_blocked(
    mut pacing: *mut picoquic_pacing_t,
) -> ::core::ffi::c_int {
    return ((*pacing).bucket_nanosec < (*pacing).packet_time_nanosec) as ::core::ffi::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_is_authorized_by_pacing(
    mut pacing: *mut picoquic_pacing_t,
    mut current_time: uint64_t,
    mut next_time: *mut uint64_t,
    mut packet_train_mode: ::core::ffi::c_uint,
    ...
    if (*pacing).bucket_nanosec < (*pacing).packet_time_nanosec {
        let mut next_pacing_time: uint64_t = 0;
        let mut bucket_required: int64_t = 0;
        if packet_train_mode != 0 || (*pacing).bandwidth_pause != 0 {
            bucket_required = (*pacing).bucket_max;
            if bucket_required > 10 as int64_t * (*pacing).packet_time_nanosec {
                bucket_required = 10 as int64_t * (*pacing).packet_time_nanosec;
            }
            bucket_required -= (*pacing).bucket_nanosec;
        } else {
            bucket_required = (*pacing).packet_time_nanosec - (*pacing).bucket_nanosec;
        }
        next_pacing_time = current_time
            .wrapping_add(1 as uint64_t)
            .wrapping_add((bucket_required / 1000 as int64_t) as uint64_t);
        if next_pacing_time < *next_time {
            (*pacing).bandwidth_pause = 0 as ::core::ffi::c_int;
            *next_time = next_pacing_time;
            if !quic.is_null() {
                (*quic).wake_file = 1 as ::core::ffi::c_int;
                (*quic).wake_line = 98 as ::core::ffi::c_int;
            }
        }
...
unsafe extern "C" fn picoquic_report_pacing_update(
    mut pacing: *mut picoquic_pacing_t,
    mut path_x: *mut picoquic_path_t,
) {
    let mut cnx: *mut picoquic_cnx_t = (*path_x).cnx as *mut picoquic_cnx_t;
    if (*cnx).is_pacing_update_requested() as ::core::ffi::c_int != 0
        && path_x == *(*cnx).path.offset(0 as ::core::ffi::c_int as isize)
        && (*cnx).callback_fn.is_some()
    {
        if (*pacing).rate > (*cnx).pacing_rate_signalled
            && (*pacing).rate.wrapping_sub((*cnx).pacing_rate_signalled)
                >= (*cnx).pacing_increase_threshold
            || (*pacing).rate < (*cnx).pacing_rate_signalled
                && (*cnx).pacing_rate_signalled.wrapping_sub((*pacing).rate)
                    > (*cnx).pacing_decrease_threshold
        {
    ...
    if (*cnx).is_path_quality_update_requested() as ::core::ffi::c_int != 0
        && (*cnx).callback_fn.is_some()
    {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_pacing_parameters(
    mut pacing: *mut picoquic_pacing_t,
    mut pacing_rate: ::core::ffi::c_double,
    mut quantum: uint64_t,
    mut send_mtu: size_t,
    mut smoothed_rtt: uint64_t,
    mut signalled_path: *mut picoquic_path_t,
) {
    let mut packet_time: ::core::ffi::c_double = send_mtu as ::core::ffi::c_double / pacing_rate;
    let mut quantum_time: ::core::ffi::c_double = quantum as ::core::ffi::c_double / pacing_rate;
    let mut rtt_nanosec: uint64_t = smoothed_rtt.wrapping_mul(1000 as uint64_t);
    (*pacing).rate = pacing_rate as uint64_t;
    ...
    (*pacing).packet_time_nanosec = (packet_time * 1000000000.0f64) as uint64_t as int64_t;
    if (*pacing).packet_time_nanosec <= 0 as int64_t {
        (*pacing).packet_time_nanosec = 1 as int64_t;
        (*pacing).packet_time_microsec = 1 as uint64_t;
    } else {
        if (*pacing).packet_time_nanosec as uint64_t > rtt_nanosec {
            (*pacing).packet_time_nanosec = rtt_nanosec as int64_t;
        }
        (*pacing).packet_time_microsec = ((*pacing).packet_time_nanosec as ::core::ffi::c_ulonglong)
            .wrapping_add(999 as ::core::ffi::c_ulonglong)
            .wrapping_div(1000 as ::core::ffi::c_ulonglong)
            as uint64_t;
    }
    (*pacing).bucket_max = (quantum_time * 1000000000.0f64) as uint64_t as int64_t;
    if (*pacing).bucket_max <= 0 as int64_t {
        (*pacing).bucket_max = 16 as int64_t * (*pacing).packet_time_nanosec;
    }
    if (*pacing).bucket_nanosec > (*pacing).bucket_max {
        (*pacing).bucket_nanosec = (*pacing).bucket_max;
    }
    if !signalled_path.is_null() {
        picoquic_report_pacing_update(pacing, signalled_path);
    }
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_pacing_window(
    mut pacing: *mut picoquic_pacing_t,
    mut slow_start: ::core::ffi::c_int,
    mut cwin: uint64_t,
    mut send_mtu: size_t,
    mut smoothed_rtt: uint64_t,
    mut signalled_path: *mut picoquic_path_t,
    ...
    let mut rtt_nanosec: uint64_t = smoothed_rtt.wrapping_mul(1000 as uint64_t);
    if cwin < (send_mtu as uint64_t).wrapping_mul(8 as uint64_t) || rtt_nanosec <= 1000 as uint64_t
    {
        (*pacing).bucket_max = rtt_nanosec as int64_t;
        (*pacing).packet_time_nanosec = 1 as int64_t;
        (*pacing).packet_time_microsec = 1 as uint64_t;
        if (*pacing).bucket_nanosec > (*pacing).bucket_max {
            (*pacing).bucket_nanosec = (*pacing).bucket_max;
        ...
        let mut pacing_rate: ::core::ffi::c_double =
            cwin as ::core::ffi::c_double / rtt_nanosec as ::core::ffi::c_double * 1000000000.0f64;
        let mut quantum: uint64_t = cwin.wrapping_div(4 as uint64_t);
        if (quantum as ::core::ffi::c_ulonglong)
            < (2 as ::core::ffi::c_ulonglong).wrapping_mul(send_mtu as ::core::ffi::c_ulonglong)
        {
            quantum = (2 as ::core::ffi::c_ulonglong)
                .wrapping_mul(send_mtu as ::core::ffi::c_ulonglong)
                as uint64_t;
        } else if slow_start != 0
            && smoothed_rtt
                > (4 as ::core::ffi::c_int * PICOQUIC_MAX_BANDWIDTH_TIME_INTERVAL_MAX) as uint64_t
        {
            let quantum_min: uint64_t = 0x8000 as uint64_t;
            if quantum < quantum_min {
                quantum = quantum_min;
            } else {
                let mut quantum2: uint64_t = (pacing_rate
                    * PICOQUIC_MAX_BANDWIDTH_TIME_INTERVAL_MAX as ::core::ffi::c_double
                    / 1000000.0f64) as uint64_t;
                if quantum2 > quantum {
                    quantum = quantum2;
                }
            }
        } else if quantum as ::core::ffi::c_ulonglong
            > (16 as ::core::ffi::c_ulonglong).wrapping_mul(send_mtu as ::core::ffi::c_ulonglong)
        {
            quantum = (16 as ::core::ffi::c_ulonglong)
                .wrapping_mul(send_mtu as ::core::ffi::c_ulonglong)
                as uint64_t;
        }
        if slow_start != 0 {
...
pub unsafe extern "C" fn picoquic_update_pacing_data_after_send(
    ...
    let mut packet_time_nanosec: uint64_t = 0;
    picoquic_update_pacing_bucket(pacing, current_time);
    packet_time_nanosec = ((*pacing).packet_time_nanosec as uint64_t)
        .wrapping_mul(length as uint64_t)
        .wrapping_add((send_mtu as uint64_t).wrapping_sub(1 as uint64_t))
        .wrapping_div(send_mtu as uint64_t);
    (*pacing).bucket_nanosec = ((*pacing).bucket_nanosec as uint64_t)
        .wrapping_sub(packet_time_nanosec) as int64_t as int64_t;
}
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_pacing_after_send(
    mut path_x: *mut picoquic_path_t,
    mut length: size_t,
    mut current_time: uint64_t,
) {
    picoquic_update_pacing_data_after_send(
        &raw mut (*path_x).pacing,
        length,
        (*path_x).send_mtu,
        current_time,
    );
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_is_sending_authorized_by_pacing(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut current_time: uint64_t,
    mut next_time: *mut uint64_t,
) -> ::core::ffi::c_int {
    return picoquic_is_authorized_by_pacing(
        &raw mut (*path_x).pacing,
        current_time,
        next_time,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_pacing_rate(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut pacing_rate: ::core::ffi::c_double,
    mut quantum: uint64_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_update_pacing_data(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut slow_start: ::core::ffi::c_int,
) {
    picoquic_update_pacing_window(
        &raw mut (*path_x).pacing,
        slow_start,
        (*path_x).cwin,
        (*path_x).send_mtu,
