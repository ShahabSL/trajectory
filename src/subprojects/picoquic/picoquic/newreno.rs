...
extern "C" {
    ...
    fn picoquic_cc_get_ack_sent_time(
        cnx: *mut picoquic_cnx_t,
        path_x: *mut picoquic_path_t,
    ) -> uint64_t;
...
pub type picoquic_newreno_alg_state_t = ::core::ffi::c_uint;
pub const picoquic_newreno_alg_congestion_avoidance: picoquic_newreno_alg_state_t = 1;
pub const picoquic_newreno_alg_slow_start: picoquic_newreno_alg_state_t = 0;
pub type picoquic_newreno_state_t = st_picoquic_newreno_state_t;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct st_picoquic_newreno_state_t {
    pub nrss: picoquic_newreno_sim_state_t,
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_newreno_sim_reset(mut nrss: *mut picoquic_newreno_sim_state_t) {
    memset(
        nrss as *mut ::core::ffi::c_void,
        0 as ::core::ffi::c_int,
        ::core::mem::size_of::<picoquic_newreno_sim_state_t>() as size_t,
    );
    (*nrss).alg_state = picoquic_newreno_alg_slow_start;
    (*nrss).ssthresh = UINT64_MAX as uint64_t;
    (*nrss).cwin = PICOQUIC_CWIN_INITIAL as uint64_t;
}
unsafe extern "C" fn picoquic_newreno_sim_enter_recovery(
    mut nr_state: *mut picoquic_newreno_sim_state_t,
    ...
    (*nr_state).ssthresh = (*nr_state).cwin.wrapping_div(2 as uint64_t);
    if (*nr_state).ssthresh < PICOQUIC_CWIN_MINIMUM as uint64_t {
        (*nr_state).ssthresh = PICOQUIC_CWIN_MINIMUM as uint64_t;
    }
    if notification as ::core::ffi::c_uint
        == picoquic_congestion_notification_timeout as ::core::ffi::c_int as ::core::ffi::c_uint
    {
        (*nr_state).cwin = PICOQUIC_CWIN_MINIMUM as uint64_t;
    ...
    (*nr_state).residual_ack = 0 as uint64_t;
}
unsafe extern "C" fn picoquic_newreno_sim_seed_cwin(
    mut nr_state: *mut picoquic_newreno_sim_state_t,
    mut path_x: *mut picoquic_path_t,
    mut bytes_in_flight: uint64_t,
) {
    if (*nr_state).alg_state as ::core::ffi::c_uint
        == picoquic_newreno_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
        && (*nr_state).ssthresh == UINT64_MAX as uint64_t
    {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_newreno_sim_notify(
    mut nr_state: *mut picoquic_newreno_sim_state_t,
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut notification: picoquic_congestion_notification_t,
    ...
    match notification as ::core::ffi::c_uint {
        0 => match (*nr_state).alg_state as ::core::ffi::c_uint {
            0 => {
                (*nr_state).cwin = (*nr_state)
                    .cwin
            ...
            1 | _ => {
                let mut complete_delta: uint64_t = (*ack_state)
                    ...
                    .wrapping_add((*nr_state).residual_ack);
                (*nr_state).residual_ack = complete_delta.wrapping_rem((*nr_state).cwin);
                (*nr_state).cwin = (*nr_state)
                    .cwin
                    .wrapping_add(complete_delta.wrapping_div((*nr_state).cwin));
            }
        },
        5 | 1 | 2 => {
            if (*nr_state).recovery_sequence <= (*ack_state).lost_packet_number {
                picoquic_newreno_sim_enter_recovery(
                    nr_state,
                    cnx,
                    path_x,
                    notification,
        ...
        3 => {
            if (*cnx).is_multipath_enabled() == 0 {
                if current_time.wrapping_sub((*nr_state).recovery_start) < (*path_x).smoothed_rtt
                    && (*nr_state).recovery_sequence > picoquic_cc_get_ack_number(cnx, path_x)
                {
                    if (*nr_state).ssthresh != UINT64_MAX as uint64_t
                        && (*nr_state).cwin < (2 as uint64_t).wrapping_mul((*nr_state).ssthresh)
                    {
                        (*nr_state).cwin = (2 as uint64_t).wrapping_mul((*nr_state).ssthresh);
                        (*nr_state).alg_state = picoquic_newreno_alg_congestion_avoidance;
                    }
                }
            } else if current_time.wrapping_sub((*nr_state).recovery_start) < (*path_x).smoothed_rtt
...
unsafe extern "C" fn picoquic_newreno_reset(
    mut nr_state: *mut picoquic_newreno_state_t,
    mut path_x: *mut picoquic_path_t,
) {
    memset(
        nr_state as *mut ::core::ffi::c_void,
        0 as ::core::ffi::c_int,
        ::core::mem::size_of::<picoquic_newreno_state_t>() as size_t,
    );
    picoquic_newreno_sim_reset(&raw mut (*nr_state).nrss);
    (*path_x).cwin = (*nr_state).nrss.cwin;
}
unsafe extern "C" fn picoquic_newreno_init(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut current_time: uint64_t,
) {
    let mut nr_state: *mut picoquic_newreno_state_t =
        malloc(::core::mem::size_of::<picoquic_newreno_state_t>() as size_t)
            as *mut picoquic_newreno_state_t;
    if !nr_state.is_null() {
        picoquic_newreno_reset(nr_state, path_x);
        (*path_x).congestion_alg_state = nr_state as *mut ::core::ffi::c_void;
    } else {
        (*path_x).congestion_alg_state = NULL;
    };
}
unsafe extern "C" fn picoquic_newreno_notify(
    mut cnx: *mut picoquic_cnx_t,
    mut path_x: *mut picoquic_path_t,
    mut notification: picoquic_congestion_notification_t,
    mut ack_state: *mut picoquic_per_ack_state_t,
    ...
    let mut nr_state: *mut picoquic_newreno_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_newreno_state_t;
    (*path_x).set_is_cc_data_updated(1 as ::core::ffi::c_uint as ::core::ffi::c_uint);
    if !nr_state.is_null() {
        match notification as ::core::ffi::c_uint {
            0 => {
                if (*nr_state).nrss.alg_state as ::core::ffi::c_uint
                    == picoquic_newreno_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
                    && (*nr_state).nrss.ssthresh == UINT64_MAX as uint64_t
                {
            ...
            3 => {
                ...
                (*path_x).cwin = (*nr_state).nrss.cwin;
                (*path_x)
                    .set_is_ssthresh_initialized(1 as ::core::ffi::c_uint as ::core::ffi::c_uint);
            }
            4 => {
                if (*nr_state).nrss.alg_state as ::core::ffi::c_uint
                    == picoquic_newreno_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
                    && (*nr_state).nrss.ssthresh == UINT64_MAX as uint64_t
                {
                    if (*path_x).rtt_min as ::core::ffi::c_ulonglong > PICOQUIC_TARGET_RENO_RTT {
                        ...
                        if min_win_0 > (*nr_state).nrss.cwin {
                            (*nr_state).nrss.cwin = min_win_0;
                            (*path_x).cwin = min_win_0;
                        }
                    }
                    if picoquic_hystart_test(
                        &raw mut (*nr_state).rtt_filter,
        ...
        picoquic_update_pacing_data(
            cnx,
            path_x,
            ((*nr_state).nrss.alg_state as ::core::ffi::c_uint
                == picoquic_newreno_alg_slow_start as ::core::ffi::c_int as ::core::ffi::c_uint
                && (*nr_state).nrss.ssthresh == UINT64_MAX as uint64_t)
                as ::core::ffi::c_int,
        );
...
unsafe extern "C" fn picoquic_newreno_delete(mut path_x: *mut picoquic_path_t) {
...
#[no_mangle]
pub unsafe extern "C" fn picoquic_newreno_observe(
    mut path_x: *mut picoquic_path_t,
    ...
    let mut nr_state: *mut picoquic_newreno_state_t =
        (*path_x).congestion_alg_state as *mut picoquic_newreno_state_t;
    *cc_state = (*nr_state).nrss.alg_state as uint64_t;
    *cc_param = if (*nr_state).nrss.ssthresh == UINT64_MAX as uint64_t {
        0 as uint64_t
    } else {
        (*nr_state).nrss.ssthresh
    };
}
pub const PICOQUIC_NEWRENO_ID: [::core::ffi::c_char; 8] =
    unsafe { ::core::mem::transmute::<[u8; 8], [::core::ffi::c_char; 8]>(*b"newreno\0") };
#[no_mangle]
pub static mut picoquic_newreno_algorithm_struct: picoquic_congestion_algorithm_t = unsafe {
    st_picoquic_congestion_algorithm_t {
        congestion_algorithm_id: PICOQUIC_NEWRENO_ID.as_ptr(),
        congestion_algorithm_number: PICOQUIC_CC_ALGO_NUMBER_NEW_RENO as uint8_t,
        alg_init: Some(
        ...
        alg_delete: Some(
            picoquic_newreno_delete as unsafe extern "C" fn(*mut picoquic_path_t) -> (),
        ),
        alg_observe: Some(
...
#[no_mangle]
pub static mut picoquic_newreno_algorithm: *mut picoquic_congestion_algorithm_t =
    unsafe { &raw const picoquic_newreno_algorithm_struct as *mut picoquic_congestion_algorithm_t };
